...
class Lista {
	...
	public boolean remover(Game x) throws Exception {
        boolean achou = false;
        if(primeiro!=ultimo){
            Celula aux = primeiro;
            while(aux.prox!=null && !achou){
                if(aux.prox.elemento.getName().compareTo(x.getName())==0){
                    achou = true;
                }
                if(!achou){
                    aux = aux.prox;
...
class HashIndiretoLista {
    ...
    public boolean remover(Game elemento) throws Exception {
        boolean resp=false;
        if (pesquisar(elemento.getName()) == false) {
