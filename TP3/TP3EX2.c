...
void game_print(Game *game)
{
   ...
   // if(game->count_genres<2) game->genres[strlen( game->genres)-1]='\0';
    // if(game->count_genres>1){
         for (int i = 0; i < game->count_genres; i++)
    {
        if(strchr(game->genres[i], '\n')!=NULL) *strchr(game->genres[i], '\n') = '\0';
            // if (game->genres[i][strlen(game->genres[i]) - 1] == '\n')
            //     game->genres[i][strlen(game->genres[i]) - 1] = '\0';
            // else if (game->genres[i][strlen(game->genres[i]) - 2] == '\n')
            //     game->genres[i][strlen(game->genres[i]) - 2] = '\0';
            // else if (game->genres[i][strlen(game->genres[i])] == '\n')
            //     game->genres[i][strlen(game->genres[i])] = '\0';
            // else if (game->genres[i][strlen(game->genres[i]-3)] == '\n')
            //     game->genres[i][strlen(game->genres[i])-3] = '\0';
...
void swap(int menor, int valor)
{
    Game aux = games[menor];
    games[menor] = games[valor];
    games[valor] = aux;
}

void selecaoRecursiva(){
    for (int i=0; i<(n-1); i++){
        int menor=i;
        for (int j=(i+1); j<n; j++){
