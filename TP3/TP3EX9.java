...
public class TP3EX9 {
    ...
    public static void main(String args[]) throws FileNotFoundException{
        ...
        //Ordenação
        listaDeGame.ordenacaoPorMergesort(0, listaDeGame.getN()-1);     
        
        //Saida
        //listaDeGame.imprimir();
        listaDeGame.mostrar(); 
    }
}
...
class Lista{
	 ...
	 */
    public void ordenacaoPorMergesort(int esq, int dir){
        //Ordenação Mergesort
        if(esq<dir){
            int meio = (esq+dir)/2;
            ordenacaoPorMergesort(esq, meio);
            ordenacaoPorMergesort(meio+1, dir);
    ...
    public void imprimir(){
        for(int i=0; i<n; i++){
            System.out.println(array[i].getUpvotes());
        }
    }
    ...
    public void intercalar(int esq, int meio, int dir){
        ...
        Game[] arrayDir = new Game[nDir+1]; //ja que eu tirei a parte da sentinela, não acrescentei 1 no tamanho do array

        //Sentinela no final dos dois arrays
        Game aux = array[0]; 
        arrayEsq[nEsq] = arrayDir[nDir] = aux; //estava dando erro nessa parte, então tirei
        int iEsq, iDir, i; 

        //Inicializar primeiro subarray - ESQUERDA
        for (iEsq = 0; iEsq < nEsq; iEsq++){
            arrayEsq[iEsq] = array[esq+iEsq];
        }

        //Inicializar segundo subarray - DIREITA
        for (iDir = 0; iDir < nDir; iDir++){
            arrayDir[iDir] = array[(meio+1)+iDir];
        }

        arrayDir[nDir] = arrayEsq[nEsq] = new Game();
        arrayDir[nDir].setUpvotes((float) 3.4028235E+38);
        arrayEsq[nEsq].setUpvotes((float) 3.4028235E+38);
        
        //Intercalacao propriamente dita
        for (iEsq = iDir = 0, i = esq; i <= dir; i++){
            for (iEsq = iDir = 0, i = esq; i <= dir; i++){
                array[i] = (arrayEsq[iEsq].getUpvotes() <= arrayDir[iDir].getUpvotes()) ? arrayEsq[iEsq++] : arrayDir[iDir++];
            }
        }
    }
}
