...
public class TP3EX6 {
    ...
    public static void main(String args[]) throws FileNotFoundException{
        ...
        listaDeGame.ordenacaoPorQuicksort(0, listaDeGame.getN()-1);     
...
class Lista{
    ...
    /**
	 * Realiza a troca de duas posições do array 
	 */
    ...
    /**
	 * Ordena a lista de Game por meio do método de Quicksort, utilizando
	 * como chave o atributo Release_date
	 */
    public void ordenacaoPorQuicksort(int esq, int dir){
        int i=esq, j=dir;
        Game pivo=array[(esq+dir)/2];
        while(i<=j){
            while(array[i].getReleaseDate().before(pivo.getReleaseDate())){
                i++;
            }
            while(array[j].getReleaseDate().after(pivo.getReleaseDate())){
                j--;
            }
            if(i<=j){
                swap(i, j);
        ...
        if(esq<j){
            ordenacaoPorQuicksort(esq, j);
        }
        if(i<dir){
            ordenacaoPorQuicksort(i, dir);
        }
    } 
}
