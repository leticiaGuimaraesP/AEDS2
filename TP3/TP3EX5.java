...
public class TP3EX5 {
    ...
    public static void main(String args[]) throws FileNotFoundException{
        ...
        listaDeGame.ordenacaoPorHeapsort();    
...
class Lista{
    ...
    /**
	 * Ordena a lista de Game por meio do método de Heapsort, utilizando
	 * como chave o atributo Release_date
	 */
    
    public void ordenacaoPorHeapsort(){
        ...
        //Ordenção
        int tam = n; //n-1 pra n pegar uma posição inexistente
        while(tam>1){
            swap(1, tam--);
            reconstruirHeap(tam);
        }
    ...
    public int getMaiorFilho(int i, int tam){
        int filho;
        if(2*i==tam || array[2*i].getReleaseDate().after(array[2*i+1].getReleaseDate())){
            filho=2*i;
        }else{
            filho=2*i+1;
        }
    ...
    public void construirHeap(int tam){
        for(int i=tam; i>1 && array[i].getReleaseDate().after(array[i/2].getReleaseDate()); i/=2){
            swap(i, i/2);
        }  
    }
