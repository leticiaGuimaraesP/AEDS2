...
public class TP2EX3 {
    ...
    public static void main(String[] args) throws Exception{
       //Leitura da primeira parte da entrada
        ...
        String[] chave = new String[1000];
        int numEntrada2 = 0;
        do {
            chave[numEntrada2] = MyIO.readLine();
        } while (isFim(chave[numEntrada2++]) == false);
        numEntrada2--;

        //Leitura do arquivo csv
        Scanner leia = new Scanner (new File("./tmp/games.csv"));
        //Scanner leia = new Scanner (new File("/tmp/games.csv"));
        String[] games = new String[5000];  //criando outro vetor, onde cada posição guarda todas as infos de um jogo
        int cont = 0;
        while(leia.hasNext()){
            games[cont] = leia.nextLine();
            cont++;
        }

        Game[] lista = new Game[numEntrada]; //criação da lista de Games
        int contGame=0;
        int index = 0;
        String app_id;

        //preenchendo a lista de Games com os jogos selecionados
        for(int i=0; i<numEntrada; i++){
            for(int j=0; j<cont; j++){
                while (true) {  //separa o id de cada jogo do arquvio
                    index++;
                    if (games[j].charAt(index) == ',') {
                        app_id = games[j].substring(0, index);
                        break;
                    }
                }
                index=0;
                if(entrada[i].equals(app_id)){ //confere se é o jogo procurado
                    lista[contGame] = new Game();
                    lista[contGame].ler(games[j]);
                    contGame++;
                    j=cont;
                }
            }
        ...
        for(int k=0; k<numEntrada2; k++){
            ...
            for(int i=0; i<lista.length; i++){
                if(chave[k].equals(lista[i].getName())){
                    achou=true;
                    i=lista.length;
                }
            }
