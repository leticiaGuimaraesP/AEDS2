        ...
        if(line[atr_index] == '\"') {
            ...
            while(true) {
                ...
                if(line[index] == ',') {
                    ...
                    else if (sub[strlen(sub)] == '\"') sub[strlen(sub)] = '\0';
...
void mostrarRec(Celula* i, int j) {
    if(i != NULL){
        j--;
        mostrarRec(i->prox, j);
...
//adicionar uma célula no fim da pilha
void empilhar(Game x) {
   Celula* aux = novaCelula(x);
   aux->prox = topo;
   topo = aux;
   aux = NULL;
}
//remover a última célula da pilha
Game desempilhar() {
   if (topo == NULL){
    printf("Erro");
   }
...
int main() {
    ...
    while(true) {
        ...
        if(found.app_id != -1) empilhar(found);
    ...
    for(int i = 0; i < n_ops; i++) {
        ...
        if(x[0] == 'I') {
            substring(y, &x[1], strlen(x) - 1);
            empilhar(game_search(atoi(y)));
            
        }
        else if(x[0] == 'R') {
            substring(y, &x[1], strlen(x)-1);
            printf("(R) %s\n", desempilhar(atoi(y)).name);
